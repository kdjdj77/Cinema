...
.gallerylist > ul {font-size:0; text-align:center;}
.gallerylist > ul > li {display:inline-block;vertical-align:top;width:300px;height:409px; margin: 50px 30px;}
...
.gallerylist > ul > li > a .screen .top {position:absolute;bottom:150%;left:8px;z-index:2;color:#fff;font-size:26px;font-weight:900;transition:all .35s;}
.gallerylist > ul > li > a .screen .bottom {position:absolute;top:150%;left:8px;z-index:2;color:#fff;font-size:18px;transition:all .35s;}
        ...
        <div class="gallerylist">
            <ul>
                <c:forEach var="list" items="${list}">
                    <li>
						<a href="dreserve?id=${list.movie.id}">
							<div class="screen">
								<div class="top">${list.movie.title }</div>
								<div class="bottom">예매한 좌석 수: ${list.seatTotal }</div>
								<c:choose>
								<c:when test="${list.fileName != null }">
								<img src="${pageContext.request.contextPath}/upload/${list.fileName}" style="width:300px; height:409px;" class="rounded border border-white">
								</c:when>
								<c:otherwise>
									<div style="background-color:gray; width:300px; height:409px; text-align:center" class="rounded">
					            		NO IMAGE
					            	</div>
								</c:otherwise>
								</c:choose>
							...
							<div>
								<h3><c:out value="${list.movie.title}"/></h3>
							</div>
						</a>
					</li>
                </c:forEach>
